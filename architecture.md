...
## Corporate Server
This server will be responsible for running services within the simulated business of the network, such as SMTP or FTP. The architecture is visualised below, with each service running in its own docker container:

![Version1 CorpSvr Architecture](https://github.com/gradyclark03/homelab-project/blob/main/screenshots/corpsvr-architecture.png)

It will run on **Ubuntu Desktop 22.04**.

### Email Server
Below is the architecture for the Email Server which runs on the Corporate Server:

![Version1 Email Architecture](https://github.com/gradyclark03/homelab-project/blob/main/screenshots/emailsvr-architecture.png)

MailHog is not a real SMTP server; it justs captures outgoing emails without delivering them to real inboxes.

The clients will run an email polling script on their machines which query the MailHog server for emails and then notifies the client when an email has been received for them.
